...
impl WebLogin {
    pub fn new(base_url: &str, account: &str, password: &str) -> Self {
        Self {
    ...
    pub fn login(
        ...
        Ok((login, is_success(&my_dbg!(end_str))))
    }
    ...
    fn get(
        ...
        self.get_with_base(&self.base_url.clone(), location, params, true)
    }
}
...
fn is_success(text: &str) -> bool {
    !text.contains("没有注册请点击这里") && !text.is_empty()
}
