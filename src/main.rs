...
fn login_wifi(ipv4: &str, account: &Account) -> Result<State, Box<dyn std::error::Error>> {
    let sign_parameter = format!(
        "http://10.2.5.251:801/eportal/?c=Portal&a=login&callback=dr1740318353616&login_method=1&user_account={}%40telecom&user_password={}&wlan_user_ip={ipv4}&wlan_user_mac={MAC_ID}&wlan_ac_ip=&wlan_ac_name=NAS&jsVersion=3.0&_=1740318341152",
        account.get_account(),
...
fn log_state(state: State, account: &Account) {
    match state {
        State::IsLogined => println!("校园网已登录: {}", account.get_account()),
        State::Success => println!("校园网登录成功: {}", account.get_account()),
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(temp_files) = self_save::get_temp(&path)
        && !temp_files.is_empty()
    {
        println!("发现未删除的临时文件");
        temp_files.iter().for_each(|f| println!("{}", f.display()));
        println!("是否删除 (y/n)");
    ...
    let mut account = if let Some(ac) = Account::load(&path) {
        ac
    } else {
        let ac = Account::get();
        println!("正在保存...");
    ...
    'outer: loop {
        if !to_offline {
            _login_state = login_wifi(&ipv4, &account)?;
            log_state(_login_state.clone(), &account);
        }
        if is_first && !is_ok {
            let r = WebLogin::login(account.get_account(), account.get_password()).unwrap_or_else(
                |e| {
                    println!("登录自助服务时出错: {:?}", e);
            ...
            is_ok = r.1;
            if is_ok {
                println!("登录自助服务成功: {}", account.get_account());
            }
            is_first = false;
        }
        if is_ok {
            ...
            println!("\n输入序号以将对应用户离线，输入r重来,s重新设置账号,输入其他以退出...");
            let mut input = String::new();
